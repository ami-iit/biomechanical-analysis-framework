...
TEST_CASE("InverseKinematics test")
{
    ...
    std::unordered_map<int, BiomechanicalAnalysis::IK::nodeData> poseNodeData;
    poseNodeData[3].I_R_IMU = I_R_IMU;
    poseNodeData[3].I_omega_IMU = I_omega_IMU;
    poseNodeData[3].I_position = I_position;
    poseNodeData[3].I_linearVelocity = I_linearVelocity;

    qInitial.setConstant(0.0);

    REQUIRE(ik.initialize(paramHandler, kinDyn));
    REQUIRE(ik.setDt(0.1));
    REQUIRE(ik.updatePoseTask(3, I_position, I_R_IMU, I_linearVelocity, I_omega_IMU));
